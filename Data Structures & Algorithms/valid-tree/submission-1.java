class Solution {
    public boolean validTree(int n, int[][] edges) {
        ...
        if(helper(n-1,  -1, arr, map)) return false;
        for(boolean b : arr){
            if(!b) return false;
        }
        return true;
    ...
    private boolean helper(int num, int parent, boolean[] arr, Map<Integer, Set<Integer>> map){
        if(arr[num]){
            return true;
        }
        arr[num]=true;
        ...
        for(int i: map.getOrDefault(num, new HashSet<>())){
            ...
            if (helper(i, num, arr, map)) return true;
        }
        return false;
    }
}
